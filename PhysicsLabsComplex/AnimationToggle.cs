...
namespace PhysicsLabsComplex
{
    public class AnimationToggle
    {
        ...
        public event Action OnAnimationCompleted; // событие завершения
            ...
            if (Reverse == false)
            {
                if (Value <= targetValue)
                {
                    ...
                    if (Value >= targetValue)
                    {
                        ...
                        OnAnimationCompleted?.Invoke(); // <-- вызываем событие
                    }
                }
            }
            else
        ...
        public AnimationToggle(string ID, ControlMethod InvalidateControl, float Value, float TargetValue)
        {
            this.ID = ID;
